...
laser_bullet_collision_sound = pygame.mixer.Sound(
    'media/bulletlasercollision.wav')
...
background_music_tracks = ['media/rocketman.ogg',
                           'media/spaceoddity.ogg', 'media/stairwaytoheaven.ogg']
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            current_background_music_track = (
                current_background_music_track + 1) % len(songs)
            pygame.mixer.music.load(songs[current_background_music_track])
            pygame.mixer.music.play()
...
fleet = Fleet(FLEET_ROW, FLEET_COLUMN, 1, enemy_image,
              GAME_LEFT_WALL + 1, GAME_TOP_WALL + 1)
hero_shield = Hero_Shield(hero_shield_image, hero.xcor, 500)
ultra_bullet = Ultra_Bullet(ultra_bullet_image, hero.xcor, 535)
hero_life_bars = Life_Bars(
    LIFE_BAR_ROW, LIFE_BAR_COLUMN, hero_first_life_image, GAME_LEFT_WALL + 125, 1)
...
while Show_Menu_Screen:
    ...
    if Selected_Option == "PLAY":
        text_start = text_format("PLAY", 'Eurostile Bold Extended', 40, WHITE)
    else:
        text_start = text_format("PLAY", 'Eurostile Bold Extended', 40, GREY)
    ...
    if Selected_Option == "QUIT":
        text_quit = text_format("QUIT", 'Eurostile Bold Extended', 40, WHITE)
    else:
        text_quit = text_format("QUIT", 'Eurostile Bold Extended', 40, GREY)
...
# Main Game Loop
while ACTUAL_GAME_IS_BEING_PLAYED:
    while START_GAME:
        handle_events()
        # | Collision |
        ...
        if len(fleet.ships) == 0:
            ...
            fleet = Fleet(FLEET_ROW, FLEET_COLUMN, FLEET_SPEED,
                          enemy_image, GAME_LEFT_WALL + 1, GAME_TOP_WALL + 1)
            LEVEL_SCORE += 1
            if LEVEL_SCORE > 10:
                clock.tick(.7)
                fleet = Fleet(0, 0, FLEET_SPEED, enemy_image,
                              GAME_LEFT_WALL + 1, GAME_TOP_WALL + 1)
                START_GAME = False
                if START_GAME == False:
                    Show_Menu_Screen = True
        ...
        pygame.display.set_caption(
            '| STAR INVASION |Score = ' + str(hero.score) + ' | Press P to pause')
